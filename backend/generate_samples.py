...
def generate_samples():
    ...
    ecommerce_data = {
        ...
        "Product Title": ["Mavi Pamuklu Tişört", "Kablosuz Kulaklık PRO", "Spor Ayakkabı", "Termos 1.5L", "Mekanik Klavye"],
    ...
    pd.DataFrame(ecommerce_data).to_excel(os.path.join(output_dir, "ornek_eticaret_urunleri.xlsx"), index=False)
    ...
    pd.DataFrame(hr_data).to_excel(os.path.join(output_dir, "ornek_ik_maas_tablosu.xlsx"), index=False)

    # 3. Banka Ekstresi
    bank_data = {
        "Tarih": [(datetime.now() - timedelta(days=i)).strftime("%Y-%m-%d") for i in range(5)],
        "Açıklama Detayı": ["Ahmet Yılmaz Kira", "Elektrik Faturası", "Maaş Ödemesi", "Amazon TR Alışveriş", "ATM Para Çekme"],
    ...
    pd.DataFrame(bank_data).to_excel(os.path.join(output_dir, "ornek_banka_ekstresi.xlsx"), index=False)
    ...
    fuel_data = {
        ...
        "Ürün Adı": ["V-Max Kurşunsuz", "Motorin PRO", "LPG Otogaz", "V-Max Kurşunsuz", "Motorin PRO"],
